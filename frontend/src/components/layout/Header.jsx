...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 transition-colors shadow-sm bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800">
              ...
              return displayNavbarCategories.map((cat, idx) => {
                ...
                return (
                  <div
                    ...
                    onMouseEnter={(e) => {
                      if (isSpecialDeal || isSpecialSeller) return;
      ...
      {/* FIXED MEGA DROPDOWN PORTAL — renders outside the overflow-clipped scroll container */}
      {hoveredCategory && (
        <div
          className="fixed z-[9999] pt-1 hidden md:block"
          style={{
            top: hoveredCategory.rect.top + 4,
            left: hoveredCategory.idx >= 9
              ? Math.max(8, hoveredCategory.rect.right - 420)
              : Math.min(hoveredCategory.rect.left, window.innerWidth - 440),
          }}
          onMouseEnter={() => {/* keep open */}}
          onMouseLeave={() => setHoveredCategory(null)}
        >
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 p-4 w-[420px] text-gray-800 dark:text-gray-200 animate-fade-in">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 pb-2.5 mb-3">
              <span className="text-[11px] font-black uppercase tracking-wider text-primary-600 dark:text-primary-400 flex items-center gap-1.5">
                <FiGrid size={13} /> {hoveredCategory.cat.name}
              </span>
              <Link
                to={`/search?category=${hoveredCategory.cat.slug}`}
                className="text-[11px] font-extrabold text-primary-600 hover:underline flex items-center gap-0.5"
            ...
            {hoveredCategory.catImg && (
              <div className="flex items-center gap-3 mb-3 p-2.5 bg-surface-50 dark:bg-gray-800/60 rounded-xl border border-gray-100 dark:border-gray-700/60">
                <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 border border-gray-200 dark:border-gray-700">
                  <img src={hoveredCategory.catImg} alt={hoveredCategory.cat.name} className="w-full h-full object-cover" />
                </div>
                <div className="min-w-0">
                  <p className="text-xs font-bold text-gray-900 dark:text-white truncate">{hoveredCategory.cat.name}</p>
                  {hoveredCategory.cat.description && (
                    <p className="text-[10px] text-gray-500 dark:text-gray-400 line-clamp-2 mt-0.5">
                      {hoveredCategory.cat.description.split(',').slice(0, 3).join(', ')}
                    </p>
                  )}
                </div>
              </div>
            ...
            {/* Subcategories grid */}
            {hoveredCategory.subs.length > 0 ? (
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                {hoveredCategory.subs.map((sub) => (
                  <Link
                    key={sub.id || sub.name}
                    to={`/search?category=${hoveredCategory.cat.slug}&q=${encodeURIComponent(sub.name)}`}
            ...
            <div className="mt-3 pt-2.5 border-t border-gray-100 dark:border-gray-800">
              <Link
                ...
                onClick={() => setHoveredCategory(null)}
              >
                Shop {hoveredCategory.cat.name} <FiChevronRight size={12} />
              </Link>
            </div>
          </div>
        </div>
